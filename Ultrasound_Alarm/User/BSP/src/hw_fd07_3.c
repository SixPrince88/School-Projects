...
{
    My_GPIO_All_Init(&fd07_3_pin_E, 1);
    My_GPIO_All_Init(fd07_3_pin_T, sizeof(fd07_3_pin_T) / sizeof(fd07_3_pin_T[0]));

    timbase_init_t tim = {.tim_x = TIM3, .arr_x = 65535, .psc_x = 7200 - 1};
    My_TIM_Base_Init(tim);
...
void FD07_3_Send_Data(void)
{
    ...
    // 包头为0xff
    My_Serial_SendByte(USART1, 0xff);
    My_Serial_SendByte(USART1, 0xff);
    // 发送数据
    for (uint8_t i = 0; i < sizeof(fd07_3_dist); i++)
    {
        My_Serial_SendByte(USART1, *(tx_data_offset++));
    }
}

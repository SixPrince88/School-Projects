...
/*
	...
	*@retval:无
*/
void My_ADC_Init(adc_init_t adc)
{
	...
	{
		if(adc.channel_group[i].channel_x==0)continue;
...
/*
	...
	*@retval:无
*/
void My_ADC_All_Init(adc_init_t adc)
{
	My_ADC_Init(adc);
	if(adc.dma_state){
		dma_init_t dma={
	...
	while (ADC_GetResetCalibrationStatus(adc.adc_x));//复位校准
	ADC_StartCalibration(adc.adc_x);
	while (ADC_GetCalibrationStatus(adc.adc_x));//校准完成

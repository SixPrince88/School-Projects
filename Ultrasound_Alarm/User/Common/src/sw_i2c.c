 ...
 * 基础操作抽象层
 **************************/
static void SW_I2C_Start(sw_i2c_OO_t* i2c)
{
...
static void SW_I2C_Stop(sw_i2c_OO_t* i2c)
{
    i2c->sda_out(LOW, i2c->user_data);
    i2c->scl_out(HIGH, i2c->user_data);
    i2c->sda_out(HIGH, i2c->user_data);
 ...
 */
static uint8_t SW_I2C_Write_Byte(sw_i2c_OO_t* i2c, uint8_t byte)
{
    uint8_t i;
    uint8_t ack;
 ...
 * @return 读取到的字节
 */
static uint8_t SW_I2C_Read_Byte(sw_i2c_OO_t* i2c, uint8_t ack)
{
    uint8_t byte = 0;
    uint8_t i;
 ...
 * @return 0:成功, 1:错误
*/
uint8_t SW_I2C_Write(sw_i2c_OO_t* i2c, uint8_t addr, const uint8_t* data, uint8_t data_length)
{
    uint8_t i;
    uint8_t ack_bit;
 ...
 * @return 0:成功, 1:错误
*/
uint8_t SW_I2C_Reg_Write(sw_i2c_OO_t* i2c, uint8_t dev_addr, uint8_t reg_addr, const uint8_t* data,uint8_t data_length)
{
    uint8_t ack_bit;
    SW_I2C_Start(i2c);
    ack_bit = SW_I2C_Write_Byte(i2c, dev_addr | I2C_WRITE);

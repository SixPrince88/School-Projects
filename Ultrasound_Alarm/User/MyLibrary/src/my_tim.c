...
/*
	...
	*@retval:void
*/
void My_TIM_Base_Init(timbase_init_t tim)
{
...
void MY_TIM_OC_Init(oc_init_t OC)
{
...
void MY_TIM_OC_Start(oc_init_t OCgroup[],char Num)
{
	for(int i=0;i<Num;i++)
    {
      if(OCgroup[i].tim_x==0)continue;
      MY_TIM_OC_Init(OCgroup[i]);
    }
}
void My_TIM_IC_Init(ic_init_t IC)
{
	...
	if(IC.mode_x==mode_ic)TIM_ICInit(IC.tim_x,&sys);
	if(IC.mode_x==mode_pwmi)TIM_PWMIConfig(IC.tim_x,&sys);
	TIM_SelectInputTrigger(IC.tim_x, TIM_TS_TI1FP1);
	TIM_SelectSlaveMode(IC.tim_x, TIM_SlaveMode_Reset);
	TIM_Cmd(IC.tim_x, ENABLE);
}
void My_TIM_IC_Start(ic_init_t ICgroup[],char num)
{
	for(int i=0;i<num;i++)
	{
		if(ICgroup[i].tim_x==0)continue;
		My_TIM_IC_Init(ICgroup[i]);
	}
}
void My_TIM_Encoder_Start(TIM_Type timx)
{
	//编码器初始化
